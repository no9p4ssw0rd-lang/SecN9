...
      setCalificaciones(res.data || {});
      // Si la carga fue exitosa, asegura que no haya un error visible
      mostrarNotificacion("Calificaciones cargadas con éxito.", "exito");
      setError(null);
    } catch (err) {
      console.error("Error detallado al cargar calificaciones:", err.response || err);
      mostrarNotificacion("No se pudieron cargar las calificaciones consolidadas de este grupo. Revisa el controlador de tu API.", "error");
      setCalificaciones({});
      // Si ocurre un error, mantén el error para detener el renderizado de la tabla.
      // En este caso, la notificación ya es suficiente, no usamos el estado 'error' aquí.
    } finally {
      setLoading(false);
    }
  };
...
      // Asegura que la calificación de la materia existe y es un número
...
    img.src = logoImage;
    
    // ⚠️ CORRECCIÓN CLAVE: Asegurar la carga de la imagen antes de usar doc.addImage
    await new Promise((resolve) => {
        img.onload = resolve;
        img.onerror = resolve; // Si falla la carga, aún intentamos continuar
    });
...
      // Asegura que cals es un array de 3 elementos o usa vacío si no existe
...
                pdfData: base64Pdf,
                fileName: `Boleta_${nombreCompleto.replace(/\s/g, '_')}.pdf`
            };
            
            // --- CAMBIO: Usar API_URL ---
